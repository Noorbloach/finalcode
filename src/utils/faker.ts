...
const fakers = {
  ...
  //   const categories = [
  //     { name: "ETA", 
  //       // tags: "Apple, Asus, Lenovo, Dell, Acer" 
  //       },
  //     {
  //       name: "Approved",
  ...
  fakeCategories() {
    ...
    // Sample 3 unique categories from the array
    const sampledCategories = _.sampleSize(categories, 4);
  
    // Map the sampled categories to include the slug
    return sampledCategories.map((category) => ({
      name: category.name,
      slug: _.replace(
        _.replace(_.toLower(category.name), / /g, "-"),
        "&",
